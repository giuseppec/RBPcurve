...
addPEV = function(obj, 
  ...
  polygon(x = c(x0[x0 > oneMinusPrev], 1, x0[x0 > oneMinusPrev][1L]),
    y = c(y1[x0 > oneMinusPrev], 1, 1), 
    border = NA, col = col)

  # Highlight the area of the probabilities when Y=0
  polygon(x = c(x0[1L], x0[x0 <= oneMinusPrev], x0[length(x0[x0 <= oneMinusPrev])]),
  ...
  if (plot.values) {
    ...
    text(x0[length(x0[x0 <= oneMinusPrev]) + 1], 1, adj = 0:1, col = text.col,
